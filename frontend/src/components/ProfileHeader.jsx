...
const ProfileHeader = ({ userData, onSave, isOwnProfile }) => {
	...
	return (
		<div className='bg-secondary shadow rounded-lg mb-6'>
			...
			>
				{isEditing && (
					<label className='bg-gray-500 hover:bg-secondary text-white py-2 px-4 rounded-full transition duration-300 flex items-center justify-center absolute top-2 right-2 bg-white p-2 rounded-full shadow cursor-pointer'>
			...
			<div className='p-4'>
				<div className='relative -mt-20 mb-4'>
					...
					{isEditing && (
						<label className='bg-gray-500 absolute bottom-0 right-1/2 transform translate-x-16 bg-white p-2 rounded-full shadow cursor-pointer'>
							<Camera size={20} className="text-accent" />
							<input
								type='file'
								className='hidden'
				...
				<div className='text-center text-white mb-4'>
					{isEditing ? (
						<input
							...
							className='bg-secondary text-2xl text-neutral font-bold mb-2 text-center w-full'
						/>
					) : (
						<h1 className='text-2xl text-white font-bold mb-2'>{userData.name}</h1>
					)}
					...
					{isEditing ? (
						<input
							...
							className='bg-secondary text-neutral text-center w-full'
						/>
					) : (
						<p className='text-white text-center'>{userData.headline}</p>
					)}

					<div className='flex justify-center items-center mt-2'>
						<MapPin size={16} className='text-accent-content mr-1' />
						{isEditing ? (
							<input
								type='text'
								value={editedData.location ?? userData.location}
								onChange={(e) => setEditedData({ ...editedData, location: e.target.value })}
								className='bg-secondary w-full text-neutral text-center'
							/>
						) : (
							<span className='text-accent-content'>{userData.location}</span>
						)}
					</div>
				</div>
