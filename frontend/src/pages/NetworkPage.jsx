        ...
        <div className="bg-secondary rounded-lg shadow p-6 mb-6">
          ...
          {connectionRequests?.data?.length > 0 ? (
            <div className="mb-8">
              ...
              <div className="space-y-4">
                {connectionRequests.data.map((request) => (
                  <FriendRequest
                    key={request._id || request.id} // Ensure a unique key for each item
                    request={request}
                  />
                ))}
              </div>
            ...
            <div className="bg-secondary border-4 border-neutral rounded-lg shadow p-6 text-center mb-6">
              ...
              <h3 className="text-2xl font-bold text-glow mb-2 text-accent">
                No Squad Invites Yet!
              </h3>
              ...
              <p className="text-gray-400 mt-2">
                ...
                and build your dream squad now!
              </p>
            </div>
          )}
            ...
            <div className="mb-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 border-t-4 border-b-4 border-neutral py-4">
                {connections.data.map((connection) => (
                  <UserCard
                    key={connection._id} // Ensure a unique key for each item
                    user={connection}
                    isConnection={true}
                  />
                ))}
